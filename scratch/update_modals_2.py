...
# Replace ThemNhieuBienTheForm return block
them_start = content.find("  return (\n    <div className=\"fixed inset-0 z-50 flex justify-center bg-black/50 backdrop-blur-sm sm:p-4 md:p-6 lg:p-8 overflow-y-auto\" onClick={onClose}>")
them_end_str = "    </div>\n  );\n}\n"
them_end = content.find(them_end_str, them_start) + len(them_end_str)

them_replacement = """  return (
    <>
      <ResponsiveModal
        isOpen={true}
        onClose={onClose}
...
if them_start != -1:
    content = content[:them_start] + them_replacement + content[them_end:]
    print("Found and replaced ThemNhieuBienTheForm")
else:
    print("Could not find ThemNhieuBienTheForm start block")
