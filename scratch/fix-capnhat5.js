...
const regex = /  const capNhatCongDoan = useCallback\(\(lenhId: string, congDoanId: string, data: any\) => \{[\s\S]*?localStorage\.setItem\(STORAGE_KEY, JSON\.stringify\(next\)\);\s*return next;\s*\}\);/m;
...
// Also replace interface type if needed
