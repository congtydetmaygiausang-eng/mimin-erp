...
export default function BangSizeManagerModal({ open, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-[#2B4C3E]/80 backdrop-blur-sm p-4 animate-fade-in">
      <div
        className="bg-[#F4F1EA] rounded-xl shadow-2xl max-w-5xl w-full max-h-[95vh] overflow-hidden flex flex-col border-4 border-[#2B4C3E]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex justify-between items-center p-4 bg-gradient-to-r from-blue-600 to-cyan-600 text-white">
        ...
        <div className="p-4 bg-white border-b flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 border-2 border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
              value={search}
          ...
          <button
            ...
            disabled={!dirty}
            className={`px-5 py-2 rounded-lg font-bold flex items-center gap-2 transition-colors shadow-md ${
              dirty
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((sp) => {
              ...
              return (
                <div
                  key={sp.id}
                  className="bg-white rounded-lg border-2 border-slate-200 p-4 hover:border-blue-300 transition-colors"
                >
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-bold text-slate-800">{sp.tenSP}</h3>
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="text-xs px-2 py-1 bg-slate-100 hover:bg-slate-200 rounded text-slate-600 font-semibold"
                      ...
                      <button
                        ...
                        className="text-xs px-2 py-1 bg-amber-50 hover:bg-amber-100 rounded text-amber-700 font-semibold flex items-center gap-1"
                      ...
                      return (
                        <div key={size} className="text-center">
                          ...
                          <input
                            ...
                            className={`w-full px-2 py-2 border-2 rounded text-center font-bold text-lg focus:ring-2 ${
                              isActive
                                ? "border-blue-400 focus:ring-blue-500 bg-blue-50 text-blue-900"
                                : "border-slate-200 focus:ring-slate-400 bg-slate-50 text-slate-400"
                            }`}
                          />
                        </div>
                      );
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-xs">
                    <div className="px-2 py-1 bg-blue-50 border border-blue-200 rounded">
                      ...
                      <span className="font-mono font-bold text-blue-900">{bangSize.ratios.join(":")}</span>
                    </div>
                    <div className="px-2 py-1 bg-emerald-50 border border-emerald-200 rounded">
                      <span className="font-semibold text-emerald-700">Rì: </span>
                      <span className="font-mono font-bold text-emerald-900">{bangSize.riSo} SP</span>
                    </div>
                    {bangSize.riSo > 0 && (
                      <div className="px-2 py-1 bg-amber-50 border border-amber-200 rounded">
                        <span className="text-amber-700">VD 100 SP = {Math.floor(100 / bangSize.riSo)} rì </span>
        ...
        {/* Footer */}
        <div className="bg-slate-100 p-3 border-t flex items-center justify-between">
          <div className="text-xs text-slate-500">
            💡 Chỉnh ratio → tự tính Rì. Click "Lưu tất cả" khi xong.
          </div>
          <button
            onClick={onClose}
            className="px-5 py-2 bg-slate-600 text-white rounded-lg font-semibold hover:bg-slate-700"
          >
            Đóng
          </button>
        </div>
