      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="px-6 py-4 bg-white border-b border-slate-100 flex flex-col gap-3">
          ...
          {lc.phanCong && lc.phanCong.length > 0 && (
            <div className="bg-slate-50/50 rounded-xl p-4 border border-slate-100 mt-2 mb-2 w-full overflow-x-auto">
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">Tiến trình đơn hàng</div>
              <div className="flex items-center min-w-max relative pb-6 pt-1 px-4">
                {/* Background track line */}
                <div className="absolute top-1/2 left-8 right-8 h-[2px] bg-slate-200 -translate-y-[14px] z-0" />
                <div className="flex items-center justify-between w-full relative z-10 gap-10">
                  {[...lc.phanCong].sort((a, b) => {
                    ...
                    return (
                      <div key={pc.id} className="flex flex-col items-center relative group min-w-[60px]">
                        <div className="relative flex items-center justify-center">
                          {isWorking && <div className="absolute inset-0 rounded-full bg-teal-400 animate-ping opacity-30" style={{ transform: 'scale(2.5)' }} />}
                          <div className={`w-3.5 h-3.5 rounded-full border-2 z-10 transition-colors duration-300 ${dotColor}`}></div>
                        </div>
                        <div className={`absolute top-6 whitespace-nowrap text-[10px] transition-all duration-300 ${textColor} ${isWorking ? 'scale-110' : ''} flex flex-col items-center gap-1`}>
