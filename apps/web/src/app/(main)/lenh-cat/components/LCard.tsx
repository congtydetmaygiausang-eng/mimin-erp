...
export function LenhCatCard({ lc, onEdit, onDelete, onChangeStatus }: {
  ...
  return (
    <div className={`bg-white rounded-[20px] p-4 shadow-sm border border-slate-200 hover:shadow-md transition-all duration-200 ${isLate ? "ring-2 ring-rose-500/40" : ""}`}>
      <div className="flex flex-col lg:flex-row gap-6">
        {/* Left Image */}
        <div 
          className="w-full lg:w-[260px] shrink-0 bg-slate-50 rounded-[14px] overflow-hidden cursor-pointer hover:opacity-90 transition-opacity flex items-center justify-center border border-slate-100"
          onClick={onEdit}
        >
          {lc.dsMau?.[0]?.img ? (
            <img src={lc.dsMau[0].img} alt="SP" className="w-full h-full object-cover min-h-[260px] lg:min-h-full" />
          ) : (
            <Shirt className="w-16 h-16 text-slate-300 my-20" />
          )}
        </div>
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          ...
          <div className="flex items-start justify-between mb-1.5">
            <span className="text-[17px] font-black text-teal-700 font-mono tracking-tight">{lc.id}</span>
            <div className="flex items-center gap-2">
              {isLate && <AlertCircle className="w-4 h-4 text-rose-600" />}
              <span className={`text-[11px] px-3 py-1 rounded-full ${s.bg} ${s.color} font-bold tracking-wide`}>
          ...
          <h3 
            className="font-black text-3xl text-slate-900 leading-tight mb-2 cursor-pointer hover:text-sky-600 transition-colors"
            onClick={onEdit}
          >
            {lc.tenSP}
          </h3>
          ...
          <div className="flex items-center gap-2 text-sm text-slate-500 font-medium mb-4">
            ...
            {lc.daCoSoDo && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 ml-2 bg-amber-100 text-amber-800 border border-amber-200 rounded text-[10px] font-black tracking-wide">
                <CheckCircle2 className="w-3 h-3" /> ĐÃ SƠ ĐỒ
              </span>
            )}
          </div>

          {/* Stats Row */}
          <div className="flex items-center justify-between border-y border-slate-100 py-3 mb-4">
            <div className="flex flex-col items-center justify-center flex-1 gap-1">
              <div className="flex items-center gap-1.5 text-[11px] text-slate-500 font-medium"><Package className="w-4 h-4 text-slate-400"/> Tổng SL</div>
              <div className="text-[17px] font-black text-slate-900 tabular-nums">{(lc.tongSL || 0).toLocaleString()}</div>
            </div>
            <div className="w-px h-10 bg-slate-200 shrink-0"></div>
            <div className="flex flex-col items-center justify-center flex-1 gap-1">
              <div className="flex items-center gap-1.5 text-[11px] text-slate-500 font-medium"><Shirt className="w-4 h-4 text-slate-400"/> Tỉ lệ size</div>
              <div className="text-base font-black text-slate-900 tabular-nums">{lc.tiLeSize || "--"}</div>
            </div>
            <div className="w-px h-10 bg-slate-200 shrink-0"></div>
            <div className="flex flex-col items-center justify-center flex-1 gap-1">
              <div className="flex items-center gap-1.5 text-[11px] text-slate-500 font-medium"><Calendar className="w-4 h-4 text-slate-400"/> Hạn</div>
              <div className="text-[15px] font-black text-slate-900 tabular-nums leading-none">
                <DateDisplay value={lc.hanHoanThanh} format="dd/MM" />
              </div>
              <div className="text-[10px] text-sky-500 font-medium leading-none mt-0.5">
          ...
          {cogs && (
            <div className="space-y-1.5 mb-4">
              <div className="text-[11px] font-black text-slate-500 uppercase tracking-widest mb-3 flex items-center gap-1.5">
                <Calculator className="w-3.5 h-3.5 text-slate-400" /> Bảng giá vốn
              </div>
              ...
              <div className="border-t border-dashed border-slate-200 pt-3 mt-3">
                ...
                <div className="flex items-center justify-between text-xs">
                  <span className="font-medium text-slate-500 pl-5.5 ml-1">Tổng lô ({lc.tongSL?.toLocaleString()} SP)</span>
                  <span className="font-black text-emerald-600 tabular-nums text-[15px]">
          ...
          {lc.phanCong && lc.phanCong.length > 0 && (
            <div className="flex flex-col gap-2 mt-auto">
              {lc.phanCong
                ...
                .map((pc, idx) => (
                <div key={idx} className="flex items-center justify-between text-xs bg-white px-3 py-2.5 rounded-lg border border-slate-200">
                  <span className="font-bold text-slate-600">{pc.tenCongDoan}</span>
                  {pc.nguoiTen ? (
                    <span className="font-bold text-slate-800">{pc.nguoiTen}</span>
                  ) : (
      ...
      {/* Footer Actions */}
      <div className="flex flex-wrap sm:flex-nowrap items-center justify-between gap-3 pt-4 mt-5 border-t border-slate-100">
        <div className="w-full sm:w-auto flex-1">
          {lc.trangThai === "DaTao" || lc.trangThai === "Nhap" ? (
            <button
              onClick={() => onChangeStatus("ChuyenTiep")}
              className="w-full sm:w-auto px-4 py-2 rounded-xl bg-violet-100 text-violet-700 hover:bg-violet-200 font-bold flex items-center justify-center gap-2 transition-colors"
            >
        ...
        <div className="w-full sm:w-auto flex items-center gap-2 justify-end">
          <button
            onClick={onEdit}
            className="flex-1 sm:flex-none text-sm px-4 py-2 rounded-xl bg-sky-50 text-sky-600 hover:bg-sky-100 font-bold flex items-center justify-center gap-1.5 transition-colors"
          >
          ...
          <select
            ...
            className="text-sm px-3 py-2 rounded-xl border border-slate-200 bg-white font-medium text-slate-700 hover:border-slate-300 focus:outline-none focus:ring-2 focus:ring-sky-500/20"
          >
          ...
          <button
            onClick={onDelete}
            className="text-rose-500 hover:bg-rose-50 p-2.5 rounded-xl border border-rose-100 hover:border-rose-200 transition-colors bg-white shadow-sm"
          >
