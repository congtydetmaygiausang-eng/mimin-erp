...
interface Props {
  xDet: any; // Thông tin xưởng dệt
  danhSachSoi: any[]; // Danh sách các loại sợi giao đi dệt
  onClose: () => void;
  autoPrint?: boolean;
}

export default function PhieuGiaoDetPrint({ xDet, danhSachSoi, onClose, autoPrint = true }: Props) {
    ...
    <>
      <style>{`
        @media print {
          ...
          #phieu-giao-det-print, #phieu-giao-det-print * { visibility: visible; }
          #phieu-giao-det-print {
            position: absolute;
            left: 0;
            top: 0;
            width: 100%;
        ...
        <div className="flex flex-1 flex-col mx-auto w-full max-w-4xl bg-slate-100 shadow-2xl h-full overflow-hidden">
          ...
          <div className="no-print flex items-center justify-between bg-slate-800 text-white px-5 py-3 shadow-md z-10">
            <div className="font-semibold flex items-center gap-2">
              ...
              IN PHIẾU GIAO DỆT GIA CÔNG
            </div>
            <div className="flex items-center gap-3">
              <button
                onClick={handlePrint}
                className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg bg-violet-600 hover:bg-violet-500 font-semibold transition"
              >
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-slate-100">
            <div id="phieu-giao-det-print" className="bg-white mx-auto shadow-lg text-slate-800 flex flex-col relative" style={{ maxWidth: "210mm", minHeight: "297mm", padding: "10mm 20mm" }}>
              {/* HEADER */}
              <div className="flex items-start justify-between border-b-2 border-violet-600 pb-4 mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-xl font-black text-violet-700 tracking-tight">POLOMIMIN</div>
                ...
                <div className="text-right">
                  <h1 className="text-2xl font-black text-violet-800 uppercase mb-1">PHIẾU GIAO DỆT</h1>
              ...
              <div className="mb-6 rounded-lg border border-slate-200 p-4 bg-slate-50 text-sm">
                <h2 className="font-bold text-slate-700 mb-2 uppercase text-xs">Thông tin Đơn vị Gia công (Xưởng Dệt)</h2>
                {xDet ? (
                  <div className="grid grid-cols-2 gap-2">
                    <div><span className="text-slate-500">Tên Xưởng:</span> <b>{xDet.ten_ncc}</b></div>
                    <div><span className="text-slate-500">Mã Xưởng:</span> <b>{xDet.ma_ncc}</b></div>
                    {xDet.dia_chi && <div className="col-span-2"><span className="text-slate-500">Địa chỉ:</span> {xDet.dia_chi}</div>}
                    {xDet.sdt && <div><span className="text-slate-500">Điện thoại:</span> {xDet.sdt}</div>}
                  </div>
                ) : (
                  <div className="text-slate-400 italic">Chưa xác định Xưởng dệt</div>
                )}
              </div>

              {/* ITEMS LIST */}
              <div className="mb-6">
                <h2 className="font-bold text-slate-700 mb-2 uppercase text-xs">Chi tiết Sợi giao dệt</h2>
                <table className="w-full border-collapse border border-slate-300 text-sm">
                  ...
                  <tbody>
                    {danhSachSoi.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="border border-slate-300 p-4 text-center text-slate-400 italic">Không có dữ liệu sợi</td>
                      </tr>
                    ) : (
                  ...
                  <tfoot>
                    <tr className="bg-slate-50 font-bold">
                      <td colSpan={5} className="border border-slate-300 p-2 text-right">TỔNG CỘNG KHỐI LƯỢNG GIAO:</td>
                      <td className="border border-slate-300 p-2 text-right text-violet-700 text-base">{tongKg.toLocaleString("vi-VN")} kg</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
              ...
              <div className="mt-auto">
                {/* CHU KY */}
                <div className="grid grid-cols-2 gap-4 text-center text-sm pt-8">
                  <div>
                  ...
                  <div>
                    ...
                    <div className="text-slate-500 italic text-[11px]">(Người giao - Ký, đóng dấu)</div>
