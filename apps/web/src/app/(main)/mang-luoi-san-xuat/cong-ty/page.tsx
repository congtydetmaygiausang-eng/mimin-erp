...
import { ArrowLeft, Building2, Check, ExternalLink, FileCheck2, FileText, Globe2, History, Images, Mail, MapPin, Phone, RefreshCw, SearchCheck, ShieldCheck, Upload, X } from "lucide-react";
import { toast } from "sonner";
import PageHeader from "@/components/ui/PageHeader";
import { discoverCompanyImages, loadCompanyDocuments, loadCompanyImages, loadCompanyProfile, reviewCompanyDocument, reviewCompanyImage, uploadCompanyDocument, type CompanyDocumentType, type ProductionCompanyDocument, type ProductionCompanyImage, type ProductionCompanyProfile, type ProductionCompanySource } from "@/lib/production-company-profile";
...
function CompanyProfileContent() {
  ...
  useEffect(() => {
    ...
    void Promise.all([loadCompanyProfile(id), loadCompanyImages(id),loadCompanyDocuments(id).catch(()=>[])]).then(([result, companyImages,companyDocuments]) => { if (active) { setProfile(result.profile); setSources(result.sources); setImages(companyImages);setDocuments(companyDocuments); } })
