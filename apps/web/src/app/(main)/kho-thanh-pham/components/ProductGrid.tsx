...
export function ProductGrid({ groups, productImages, setUploadingSP, setUploadType, fileInputRef, setShowAdd, setShowMasterDetails, setEditing, handleXuatKho, update, dsSanPham, onDangBan }: ProductGridProps) {
  return (
...
function VariantCard({ sp, image, onUploadImage, onEdit, onXuatKho }: { sp: SanPhamTP; image?: string; onUploadImage: () => void; onEdit: () => void; onXuatKho: () => void }) {
  return (
    <div className="flex flex-col w-32 sm:w-36 group">
      <div
        className="w-full aspect-square rounded-t-xl overflow-hidden border-2 border-slate-200 group-hover:border-emerald-400 transition-colors bg-white relative cursor-pointer"
      ...
      <div className="w-full bg-white border-x-2 border-b-2 border-slate-200 group-hover:border-emerald-400 rounded-b-xl px-2 py-2">
        <div className="font-black text-slate-800 text-sm truncate">{sp.mau}</div>
        <div className="text-[10px] text-slate-500 font-bold uppercase mt-0.5">{sp.size}</div>
        <div className="flex items-center justify-between mt-1.5">
          <span className="font-black text-amber-600 text-sm">{sp.soLuong.toLocaleString()}</span>
        ...
        <div className="flex items-center gap-1 mt-1 flex-wrap">
          {sp.trangThai === "con" && <span className="px-1.5 py-0.5 bg-emerald-100 text-emerald-700 text-[9px] rounded font-bold">Còn</span>}
          {sp.trangThai === "dat-hang" && <span className="px-1.5 py-0.5 bg-blue-100 text-blue-700 text-[9px] rounded font-bold">Đang SX</span>}
          {sp.trangThai === "xuat-kho" && <span className="px-1.5 py-0.5 bg-slate-200 text-slate-700 text-[9px] rounded font-bold">Đã xuất</span>}
          {sp.trangThai === "khong-dat" && <span className="px-1.5 py-0.5 bg-rose-100 text-rose-700 text-[9px] rounded font-bold">Không đặt</span>}
        </div>
        <div className="text-[9px] text-slate-400 font-semibold flex items-center gap-0.5 mt-1">
          <Box className="w-2.5 h-2.5" /> {sp.viTri}
        </div>
      </div>
    </div>
  );
