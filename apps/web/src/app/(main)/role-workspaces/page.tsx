...
import {
  Factory, Package, Truck, Palette, Boxes, ClipboardCheck, Calculator, Shield,
  ChevronRight, Plus, Eye, Sparkles, GitBranch, Layers, TrendingUp, AlertCircle,
  CheckCircle2, BarChart3, FileText, Users, ArrowRight, Star, Lock,
} from "lucide-react";
...
import {
  ...
  type PhieuNhapSoi, type LenhDet, type MeNhuom, type LoVaiTP, type MauNhuom, type NghiemThuMau,
} from "@/lib/yarn-production-chain";
import {
  buildMeSoiTongQuan, goiYTachLenhDet, goiYGopMeNhuom, pheDuyetQCMeSoi,
  getAllMeSoi, type MeSoiTongQuan, type ChatLuongMeSoi,
} from "@/lib/yarn-me-soi-engine";

type Role =
  | "kho-soi"      // Quản lý kho sợi
  | "xuong-det"    // Xưởng dệt
...
const ROLES: { key: Role; label: string; icon: any; color: string; moTa: string }[] = [
...
// ============ KHO SỢI WORKSPACE ============
function KhoSoiWorkspace({ user }: any) {
...
// ============ XƯỞNG DỆT WORKSPACE ============
function XuongDetWorkspace({ user }: any) {
  ...
  // Tự động gợi ý tách lệnh khi chọn lô sợi
  useEffect(() => {
    if (form.maLoSoi) {
...
// ============ XƯỞNG NHUỘM WORKSPACE ============
function XuongNhuomWorkspace({ user }: any) {
  ...
  // Gợi ý gộp mẻ nhuộm khi chọn lô mộc
  useEffect(() => {
    if (form.maLoMoc) {
      // Extract mã lô sợi từ LM-LD_001 → LD_001
...
// ============ KHO TP WORKSPACE ============
function KhoTPWorkspace({ user }: any) {
...
function TimelineRow({ label, value, status }: { label: string; value: string; status: "ok" | "warn" | "fail" }) {
  return (
...
// ============ HELPERS ============
function WorkspaceHeader({ title, subtitle, color, action }: any) {
  return (
...
function LinkToMeSoi({ maLo, small = false }: { maLo: string; small?: boolean }) {
  return (
